...
| 🚫 **移除菜单栏图标** | 不再在顶部状态栏显示托盘图标；开关/切换/退出统一用**右键桌宠**。另可跑 `make-launcher.sh` 生成可点击的**「桌宠开关.app」**（双击智能开关，见安装第 3 步）|
| 🚫 **关闭问题自启** | 官方 `install` 会开启开机自启，但其 `auto-launch` 在 macOS 上有 bug：开机会弹出一个 **Node.js REPL 窗口**。本包默认帮你关闭自启 |
| 🆙 **install 不再清零等级** | 官方 `install <新宠物>` 会把全局等级/经验**重置为 0**，还会**重开自启**。本包改写 `install.js`：装新宠物时**保留等级/经验**（只切换当前宠物）、且**不再重开自启** |
...
跑完 `apply.sh` 桌宠就会自动出现在右下角；之后**右键桌宠**即可切换/隐藏/退出。

### 3.（可选）生成一个可双击的启动器 App

不想每次敲 `petdex-cc start` 的话，生成一个可点击的「桌宠开关.app」(双击 = 智能开关)：

```bash
chmod +x make-launcher.sh
./make-launcher.sh
```

它用 `osacompile` 生成**原生通用二进制**（不需要 Rosetta）、装上爪印图标，放进「应用程序」。之后在 Launchpad / Spotlight 搜「桌宠」双击即可。首次双击若提示「未验证开发者」→ 右键 App → 打开 → 确认（仅一次）。仅 macOS。
...
> ✅ **`install` 的两个官方副作用已被本包修复**（前提是已 `./apply.sh`）：
> 1. ~~重置全局等级经验为 0~~ → 现在**保留等级/经验**，只切换当前宠物
> 2. ~~重新打开开机自启（弹 node REPL）~~ → 现在**不再重开自启**
>
> 所以装新宠物可以放心用 `install`，等级不会清零。已装过的宠物用 `switch` 切换即可（无需重新下载）。
...
├── apply.sh                      # 一键应用魔改（含备份）
├── restore.sh                    # 一键还原官方
├── make-launcher.sh              # (可选) 生成可双击的「桌宠开关.app」启动器
├── assets/
│   └── app-icon.png              # 启动器 App 图标（爪印渐变）
├── hooks/
...
└── patches/
    ...
    ├── cli/install.js            # install 保留等级 + 不重开自启
